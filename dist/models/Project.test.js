...
describe('Project', function () {
    ...
    beforeEach(async function () {
        ...
        user2 = await User_1.default.create('user2@example.com', 'User2', '12345');
        user3 = await User_1.default.create('user3@example.com', 'User3', '12345');
    ...
    describe('#getById', async function () {
        it('gets a project by id', async function () {
            const project = await Project_1.default.getById(project1.id);
            chai_1.assert.equal(lodash_1.get(project, 'id'), project1.id);
        });
    });
    describe('#deleteAll', async function () {
        it('deletes all projects', async function () {
            const numDeleted = await Project_1.default.deleteAll();
    ...
    describe('#getUsers', async function () {
        ...
        it('returns 3 users', async function () {
            ...
            chai_1.assert.lengthOf(users, 3, 'has length of 3');
        });
    });
    describe('#getUserById', async function () {
        ...
        it('returns project user', async function () {
            ...
            chai_1.assert.strictEqual(lodash_1.get(user, 'membershipType'), 'author');
            chai_1.assert.strictEqual(lodash_1.get(user, 'membershipIsActive'), true);
            chai_1.assert.strictEqual(lodash_1.get(user, 'id'), user1.id);
        });
    });
    ...
    describe('#getUsersByMembershipType', async function () {
        ...
        it('returns 2 users', async function () {
            ...
            chai_1.assert.lengthOf(users, 2, 'has length of 2');
        });
        it('returns 1 users', async function () {
            const users = await project1.getUsersByMembershipType('admin');
            chai_1.assert.lengthOf(users, 1, 'has length of 1');
        });
    });
    describe('#addUser', async function () {
        it('adds user to project', async function () {
            ...
            chai_1.assert.isFalse(isMember, 'user is not member');
            await project1.addUser(user2, 'author');
            isMember = await project1.isMember(user2.id);
            chai_1.assert.isTrue(isMember, 'user is member');
        });
    });
    ...
    describe('#getActiveWebHooks', async function () {
        beforeEach(async function () {
            await WebHook_1.default.query().insert({ name: 'webHook1', url: 'http://example.com', isActive: true, projectId: project1.id });
            await WebHook_1.default.query().insert({ name: 'webHook2', url: 'http://example.com', isActive: true, projectId: project1.id });
            await WebHook_1.default.query().insert({ name: 'webHook3', url: 'http://example.com', isActive: false, projectId: project1.id });
        });
        it('project has 2 active webooks', async function () {
            let webHooks = await project1.getActiveWebHooks();
            chai_1.assert.lengthOf(webHooks, 2, 'has length of 2');
        });
    });
    describe('#updateUserMembership', async function () {
        ...
        it('changes an existing users membershipType', async function () {
            ...
            const users = await project1.getUsers();
            chai_1.assert.lengthOf(users, 1);
            chai_1.assert.strictEqual(users[0].membershipType, 'admin');
            chai_1.assert.isTrue(users[0].membershipIsActive);
        });
        it('sets membershipIsActive to false', async function () {
            let isActiveMember = await project1.isActiveMember(user2.id);
            chai_1.assert.isTrue(isActiveMember);
            await project1.updateUserMembership(user2.id, false);
            isActiveMember = await project1.isActiveMember(user2.id);
            chai_1.assert.isFalse(isActiveMember);
            const users = await project1.getUsers();
            chai_1.assert.lengthOf(users, 1);
            chai_1.assert.strictEqual(users[0].membershipType, 'author');
            chai_1.assert.isFalse(users[0].membershipIsActive);
        });
    });
});
