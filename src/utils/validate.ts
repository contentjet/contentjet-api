...
validate.validators.lessThanAttribute = (
  value: any, options: string, _key: string, attributes: object
): string|undefined => {
...
validate.validators.boolean = (value: any): string|undefined => {
  if (!validate.isDefined(value)) return;
  if (!validate.isBoolean(value)) return 'must be true or false';
  return undefined;
};

validate.validators.arrayLength = (value: any, options: any): string|undefined => {
  if (!validate.isDefined(value)) return;
...
validate.validators.arrayOfIds = (value: any): string|undefined => {
  if (!validate.isDefined(value)) return;
  if (!validate.isArray(value)) return 'must be an array';
...
validate.validators.arrayOfStrings = (value: any): string|undefined => {
  if (!validate.isDefined(value)) return;
  if (!validate.isArray(value)) return 'must be an array';
...
validate.validators.uniqueArray = (value: any): string|undefined => {
  if (!validate.isDefined(value)) return;
  if (!validate.isArray(value)) return 'must be an array';
...
validate.validators.choicesUnion = (value: any, options: { choices: string[] }): string|undefined => {
  if (!validate.isDefined(value)) return;
  if (!validate.isArray(value)) return 'must be an array';
...
validate.validators.media = async (value: any, options: { projectId: number }) => {
  if (!validate.isDefined(value)) return;
...
validate.validators.entries = async (value: any, options: { projectId: number }) => {
  if (!validate.isDefined(value)) return;
  const { projectId } = options;
  if (!validate.isArray(value)) return 'must be an array';
  const ids = [];
...
validate.validators.projectMember = async (value: any, options: { project: Project }) => {
  if (!validate.isDefined(value)) return;
...
validate.validators.tags = (value: any): string|undefined => {
  if (!validate.isDefined(value)) return;
  if (!validate.isArray(value)) return 'must be an array';
  for (const item of value) {
